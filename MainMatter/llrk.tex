...
Para cada ecuación de prueba se resume en una tabla los resultados de la integración con los códigos antes mencionados. La precisión de cada código se mide por el error relativo
...
Hey que tener en cuenta que en el caso de que se modifique el valor del tamaño de paso $h_{n}$, la evaluación de $f$ y $f_x$ en $y_n$ realizada para calcular $\overline{H}$ es reutilizada. Por lo tanto, este reemplazo de tamaño de paso no se considera como un paso fallido ``típico" .
...
\begin{algorithm}[h]
	...
	\eIf{$fail=false$}{
		\If{$n>0$}{
			Compute the relative dissimilarity measure $\varepsilon_J$ by (\ref{jerroraprox}) \\
	...
	\While{work}{
		...
		\eIf{$\varepsilon_{K}/\gamma_{K}< 1$}{
			...
			\eIf{$breakdown$ = $true$ \textbf{or} $\mf_n=\mf_{max}$}{
				...
				(\ref{calcmnew}) \\
				Run Algorithm~\ref{alg:Arnoldiexpand} to get  matrices $H^*_{m_{new}}$ and $V_{m_{new}}$ of $\mathcal{K}_{\mf_{new}}(h_nJ,f)$ and set $H_{m_{new}}=H^*_{m_{new}}/h_n$\\
				Set $\mf_n=\mf_{new}$ and update all the variables depending of $\mf_n$,
			}
		}
...
\begin{algorithm}[h]
	...
	Compute $h_0$ by formula (\ref{hcero}) \\
	$J = f_x(y_n)$ \\
	$J_{old} = J$, $freshJ=true$ \\
	$f_{old} = f(y_n)$ \\
	\While{$t_n<T$}{
		Jacobian reuse control by Algorithm~\ref{alg:jaccontrol} \\
		Step size control as specified in Section~\ref{hcontrol} \\
		\If{$fail=false$ or $reuse=false$}{
			Run Algorithm~\ref{alg:Arnoldi} to get matrices $H^*_{m_n}$ and $V_{m_n}$ of $\mathcal{K}_{\mf_n}(h_nJ,f)$ and set $H_{m_n}=H^*_{m_n}/h_n$ \\
		}
		Computation of $\widetilde{u}_j$ by Algorithm~\ref{alg:errcontrol}\\
		Evaluation of the embedded formulas (\ref{LLDPK scheme}) \\
		Estimation of the embedded formulas error (\ref{epsilon_y}) \\
		Estimation of the new Krylov dimension $\mf_{new}$ by formula (\ref{calcmnew}) \\
		Estimation of the new step size $h_{new}$ by formula (\ref{hnewcalc}) \\
...
\begin{figure}
	...
	\vspace{-0.75in}
	\caption{ Diagrama log-log comparativo de precisión frente a tiempo para cada uno de los códigos en la integración de las seis ecuaciones de prueba. $\vartriangle$: code \emph{exp4}, $\circ$ : code \emph{ode15sk}, and $\square$: code \emph{LLDP45}}
