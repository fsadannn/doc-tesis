...
\importantdefinition{Discretización embebida Lineal Local Runge-Kutta de Dormand and Prince}
...
A continuación, para EDO de dimensiones no pequeñas, las aproximaciones Krylov-Padé propuestas en el capítulo anterior serán utilizadas para aproximar los términos $u_j$ que aparecen en (\ref{lldis}). Para ello los $u_j$ que aparecen en (\ref{lldis}) se reescriben como
\begin{equation}
...
\begin{algorithm}[htb]
	...
	\While{$t_n \le T$}{
        ...
        Evaluar la fórmula de orden $r$ en (\ref{LLDPK scheme})~(i.e., $\widehat{y}_{n+1}$ o $y_{n+1}$) \\
	    Estimar nueva dimensión de Krylov $\mf_{new}$ mediante fórmula (\ref{calcmnew})\\
...
\importantcodes{LLDP4: esquema Runge-Kutta Localmente Linealizado de Dromand y Prince de orden 4 y paso fijo}
\importantcodes{LLDP5: esquema Runge-Kutta Localmente Linealizado de Dromand y Prince de orden 5 y paso fijo}
...
\importantcodes{LLDP45: formulas embebidas Runge-Kutta de Dormand y Prince localmente linealizadas}
...
\importantcodes{ode15s: fórmulas diferenciales hacia atrás de orden variable 1-5}
...
\importantcodes{exp4: integrador exponencial de orden 4}
