...
matriz cuadrada, $F: \mathbb{R}^d\to\mathbb{R}^d$ una función vectorial no lineal de $x(t)\in\mathbb{R}^d$ y $t\geq t_0 \in \mathbb{R}$. Típicamante, la matriz $A$ en (\ref{ODE Lines Method}) resulta mal condicionada, lo que establece un determinado nivel de rigidez (stiffness en inglés) para ese sistema de ecuaciones \cite{schiesser2012}. La rigidez del sistema (\ref{ODE Lines Method}) se incrementa con la disminución de la distancia entre los puntos de la discretización del método de líneas o con el aumento del número ecuaciones, lo que dificulta su integración con esquemas numéricos explícitos tradicionales \cite{schiesser2012}. Más recientemente, a finales de los años 90, la clase de integradores exponenciales fue extendida a EDO rígidas (stiff en inglés) de la forma general (\ref{ODE-SYST}).
...
%	\qtreeshowframes
	\begin{center}
		...
		\brOverride
		\leaf{ \tikzmark[top]{a} $\widetilde{y}_{n+1} = \widetilde{u}(t_{n+1};\widetilde{y}_n)+\widetilde{r}(t_{n+1};\widetilde{y}_n)$  \tikzmark[top]{b}\\
		$\widetilde{y_0}=x(t_0)$}
		\branch{1}{ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\}
		\brRestore
		...
		\brOverridedd
		\branch{3}{$ x(t_{n+1}) = u(t_{n+1};x(t_n))+r(t_{n+1};x(t_n))$}
		\brRestore
...
\begin{definition}
	\label{definition LLS} Dada la discretización Lineal Local 
	  $y_{n+1}=y_{n}+\digamma(t_{n},y_{n},t-t_{n})$
...
y potenciación para matrices pequeñas y densas, y el de los subespacios de Krylov para matrices grandes y ralas.
...
\begin{algorithm}
    ...
    \label{alg:Arnoldi}
    \KwIn{Matriz $A\in \mathbb{R}^{d\times d}$, vector $b\in \mathbb{R}^{d}$ y dimensión del subespacio de Krylov $\mf$}
...
\end{equation*}
expresion que se deriva de propiedad de invarianza ante escalado de las bases ortonormales de Krylov. Esta propiedad de invarianza se refiere al siguiente resultado. Si $V^*_m$ y $H^*_m$ denotan a la base ortonormal y a la matriz de Hessenberg resultante del Algoritmo de Arnoldi \ref{alg:Arnoldi} para el subespacio de Krylov $\mathcal{K}_\mf(\tau A,b)$, entonces
\[ V^*_m=V_m       \text{   y    }      H^*_m=\tau H_m,\]
donde $V_m$ es la base ortonormal y $H_m$  la matriz de Hessenberg resultante del Algoritmo de Arnoldi \ref{alg:Arnoldi} para el subespacio de Krylov $\mathcal{K}_\mf(A,b)$.
