...
\begin{algorithm}
    ...
    \KwOut{$V_\mf=\left[ v_1 \, \cdots \, v_\mf \right]\in\mathbb{R}^{d\times\mf}$, matriz de Hessenberg superior $H_\mf^*=V_\mf^{\intercal}A V_\mf $, $v_{\mf}\in\mathbb{R}^d$, $\hf^*_{\mf+1,\mf}$, $\mf_{cut}$, $breakdown$ }
    ...
    \For{ $j=1,2,\ldots,\mf$ }{
        $w_j= A v_j$\\
        \For{ $i=1,\ldots,j$}{
            $\hf^*_{ij}=\langle w_j,v_i \rangle$\\
            $w_j=w_j-\hf^*_{ij}v_i$
        }
        $\hf^*_{j+1,i}=\lVert w_j \rVert_2$\\
        \eIf(\tcp*[h] \emph{Break Down}\label{alg:breakdown}){$\hf^*_{j+1,i}<2\epsilon_{mach}$}{
            $\mf_{cut}=j$\\
             $breakdown=true$\\
            Stop
        }{
            $v_{j+1}=w_j/\hf^*_{j+1,i}$
        }
    }
