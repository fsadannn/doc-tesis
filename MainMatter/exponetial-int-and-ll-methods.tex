...
Differencing y Exponential Runge-Kutta (ver \cite{Berland07} para una revisión). Por otra parte, después de la linealización local~(\bluemark{Sección \ref{section:ll-methods}}) de la ecuación (\ref{ODE-SYST}) en $t_{n}$,  la fórmula de variación de constantes lleva a
\begin{eqnarray}
x(t_{n}+h) &= & x(t_{n})+\int\limits_{0}^{h}e^{A_{n}(h-s)}(f(t_n,x(t_{n}))+sf_t(t_n,x(t_{n})))ds + \bluemark{r(t_n,y_n,h)} \label{VCF-full} \\
\bluemark{r(t_n,y_n,h)} & = & \int\limits_{0}^{h}e^{A_{n}(h-s)}g_{n}(t_{n}+s,x(t_{n}+s))ds, \label{VCF}
\end{eqnarray}
...
%Otros  integradores que provienen de esta aproximación son~\cite{Jimenez02AMC,Jimenez05AMC}.
 ...
 Obsérvese que $\phi(t_{n},x(t_n);h_n)$ en la discretización Lineal Local (\ref{ODE-LLA-4}) representa la primera integral de la \bluemark{expresión (\ref{VCF-full})}. De esta forma la integral de la expresión (\ref{VCF}) representa el residuo de dicha aproximación a la solución de la ecuación diferencial (\ref{ODE-SYST}).
...
Estos métodos de Linealización Local de Orden Superior se obtienen de agregar a la aproximación Lineal Local~(\ref{ODE-REV-FORM-LLA}) un término adicional $r$ que resulta de aproximar, con un orden superior a 2, la integral de la expresión~(\ref{VCF}) por algún método de cuadratura o por la solución numérica de la representación diferencial de la integral~(\ref{VCF})~\cite{delaCruz06}. \bluemark{Utilizando} diferentes tipos de cuadraturas para aproximar~(\ref{VCF}) se obtienen los métodos Exponenciales de Propagación 
...
\label{ODE-REV-HOLL}
\end{equation}
donde ${n_{t}=\maxx{n:t_{n}\leq t}}$, $r(t,t_{n_{t}},y_{n_{t}})$ es la solución numérica de orden $\gamma >2$ de la \bluemark{representación diferencial de la integral~(\ref{VCF})}
\begin{eqnarray}
...
\bluemark{
	\begin{center}
	\leaf{\small Aproximación numérica}
	...
	\branch{1}{\small EDO no lineal asociada a $r$\\$\frac{dr(t)}{dt} = q(x(t_n);t,r(t))$\\$r(t_n)=0$\\
		...
		\hskip 1.0in $-f_x(x_n)(u(s;x_n)-x_n)$\\
		\hskip 0.14in$-f(x_n)$
	}
	\branch{2}{$ x(t_{n+1}) = u(t_{n+1},x_n)+r(t_{n+1},x(t_n)),\; t\in[t_{n},t_{n+1}]$}
	\branch{1}{\footnotesize $\frac{dx}{dt}=f(x(t_n))+f_x(x(t_n))(x(t)-x(t_n))+\left[ f(x(t))-f(x(t_n))-f_x(x(t_n))(x(t)-x(t_n))  \right]$\\$x(t_0)=x_0$}
	\branch{1}{$\frac{dx}{dt}=f(x),\; x(t_0)=x_0$}
	\qobitree
\end{center}
}
...
Cuando la norma de \mmark{una} \redmark{matriz cuadrada} $A$ es grande, el costo computacional y los errores de redondeo \mmark{en $P_{\pf,\qf}$} aumentan, por lo que
