...
\begin{eqnarray*}
q(t_n,y_n,s,\varsigma)&=&f(s,y_n+\varphi(t_n,y_n,s-t_n)+\varsigma)-f_x(t_n,y_n)\phi(t_n,y_n,s-t_n)-f(t_n,y_n)%\\
...
Si se utiliza la fórmula de Padé para aproximar $\varphi_j$ que aparecen en las discretizaciones (\ref{LL-scheme-4}) y (\ref{LLDP-scheme-dp}) se obtienen los esquemas propuestos en \cite{Jimenez13,Jimenez14AMC} que están orientados a resolver problemas de valor inicial de dimensiones pequeñas. 
...
En general, para la familia de funciones $\phi_k$ se establece la expansión en serie \cite{Saad92,sidje1998expokit}
\begin{equation}
