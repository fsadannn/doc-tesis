		...
		\While{work}{
			...
			\eIf{$\varepsilon_{K}/\gamma_{K}< 1$}{
				...
				\eIf{$breakdown$ = $true$ \textbf{or} $\mf_n=\mf_{max}$}{
				    \textcolor{red}{Calcular $h_{new}$ como en (\ref{hnew})} \\
...
\textcolor{red}{
	...
	\[ \overline{h}_* = \me{\dfrac{ \log{\gamma} - \log{c_{r,n}} -\log{e_{\mf_n}^T H_{\mf_n}^{\mf_n-1} e_1}+\sum\limits_{j=2}^{r+\mf_n-2}\log{j}}{(r+\mf_n-2)}}. \]
...
\begin{equation*}
    \Delta = \begin{cases}
        ...
        \end{cases},
\end{equation*}
con  $\rho = 1\mathord{.}25 \left( \frac{\varepsilon_y}{RTol} \right)^{1/\textcolor{red}{5}}$, siendo
...
\begin{table}[htb]
	\caption{Valores mínimos y máximos de $\alpha _{1}$ y $\alpha _{2}$ correspondientes a los valores de estos parámetros en el esquema \emph{LLDP2} al integrar las seis ecuaciones de prueba. $r^{min}-r^{max}$ es el rango de fluctuaciones en el orden de convergencia $r$ del integrador \emph{LLDP2} durante el proceso de integración. \textcolor{red}{El símbolo} $+$ y $*$ indican, respectivamente, intercambios de fórmulas de diferencias finitas en la expresión (\ref{Jacobian-free LLDPK scheme}) durante el proceso de integración.}
	\label{tab:R_LLDP2}
	\begin{adjustbox}{width=0.95\columnwidth,center}
		\begin{tabular}{lccccccccccc}
			& \multicolumn{3}{c}{$\alpha^{min}_{1}-\alpha^{max}_{1}$} &  & \multicolumn{3}{c}{$\alpha^{min}_{2}-\alpha^{max}_{2}$} &  & \multicolumn{3}{c}{$r^{min}-r^{max}$} \\
