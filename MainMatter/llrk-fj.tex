...
En el caso de que se utilize un esquema Runge-Kutta de orden $r$ con $s$ estados en (\ref{JFKPHOLL}) para aproximar el PVI no lineal (\ref{fjsyst}), se obtiene el esquema Runge-Kutta Localmente Linealizado (LLRK) libre de Jacobiano
...
\end{equation*}
$\widetilde{k}_1\equiv 0$, $c=[0,1/2,1]$, y condición de orden  (\ref{order condition}) with $r=3$; y el esquema LLRK libre de Jacobiano de cuarto orden
\begin{equation}
...
En este conjunto de simulaciones, se utilizaran los PVI resulantes de la discretización espacial de cuatro EDP empleadas con frecuencia en la literatura. Estas ecuaciones son: Brusselator 2D (Ejemplo \ref{ex:Brus}), Brusselator 2D (Ejemplo \ref{ex:Brus2D}), Burger's (Ejemplo \ref{ex:Burger}) y Gray-Scott 2D~(Ejemplo \ref{ex:GS2D}).
...
En esta sección, se propone una implementación adaptativa de las fórmulas embebidas de Runge-Kutta localmente linealizadas libres de Jacobiano (\ref{Jacobian-free LLDPK scheme}). La implementación general del esquema libre de Jacobiano de orden variable con tamaño de paso, dimensión de Krylov y orden de Padé variables se esboza en Algorithm \ref{alg:integrator-fj}. Este Algoritmo es una versión adaptada del Algoritmo \ref{alg:integrator}. A continuación se detallan las particularidades de esta implementación libre de Jacobiano.

{\SetAlgoNoLine
	\begin{algorithm}[htb]
		\caption{Variable-order Jacobian-free scheme with variable step size, Krylov dimension y Padé order}
...
En el capítulo anterior, para la dimensión del subespacio de Krylov $\mathfrak{m}_{new}$ que se utlizará en el próximo paso de integración, se utilizó la expresión (\ref{calcmnew}), pero con $\varepsilon_{K} = \epsilon _{4}$ en lugar de (\ref{errrel-fj}) y con la aproximación
$\widehat{K}_{\mathfrak{m},k}^{\mathfrak{p},\mathfrak{q}}\left(
c_{j}h_{n},f_{x}(y_{n}),f(y_{n});\eta _{1},\delta _{1},\beta \right)
