...
\importantcodes{BDF4: fórmula diferencial hacia atrás de orden 4 libre de Jacobiano y paso fijo}
\importantcodes{JF-Exp4: integrador exponencial de orden 4 libre de Jacobiano y paso fijo}
\importantcodes{JF-EPIRK4: integrador exponencial de propagación iterativa tipo Rungge-Kutta de orden 4 libre de Jacobiano y paso fijo}
...
{\SetAlgoNoLine
	\begin{algorithm}[htb]
		...
		\While{$t_n<T$}{
			Calcular $\delta_1$ y $\delta_2$ mediante (\ref{deltavalue}), y determinar $g_1$ y $g_2$ según (\ref{g1}) y (\ref{g2}) \label{set delta}\\
			\If{$fail=false$}{
...
\subsection{Experimentos numéricos}
\importantcodes{LLDP1: formulas embebidas Runge-Kutta de Dormand y Prince localmente linealizadas libres de Jacobiano}
...
\importantcodes{LLDP2: formulas embebidas Runge-Kutta de Dormand y Prince localmente linealizadas libres de Jacobiano con diferencia finita de orden 1 en la parte lineal}
\importantcodes{ode15sk: fórmulas diferenciales hacia atrás de orden variable 1-5 libres de Jacobiano}
\importantcodes{exp4jf: integrador exponencial de orden 4 libre de Jacobiano}
