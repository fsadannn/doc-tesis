...
Es importante destacar que el teorema \ref{theorem:fj-llrk-convergence} extiende el resultado de convergencia del Teorema 15 en \cite{Jimenez13} a la familia de esquemas libres de jacobiano Localmente Linealizados de Orden Superior. Cuando la matriz jacobiana se toma exacta, los dichos esquemas convierten en esquemas ordinarios Localmente Linealizados de Orden Superior, y el resultado del Teorema \ref{theorem:fj-llrk-convergence} se reduce al del Teorema 15 en \cite{Jimenez13}.
...
\section{Esquemas Dormand y Prince}
En este capítulo se extenderán los resultados esquemas basados en Krylov-Padé libre de Jacobiano a fórmulas embebidas similar al Capítulo \ref{chapter:lldp}.

Utilizando la aproximación $(\mf , \pf ,\qf , k)$-Krylov-Padé Libre de Jacobiano~(\ref{eq:gen_kp_aprox_fj}) para $u_j$ en las fórmulas embebidas (\ref{lldis}), obtenemos la Fórmulas de Runge-Kutta localmente linealizadas libres de Jacobiano
\begin{equation}
...
\begin{theorem}\label{Teorema Convergencia}
	Sea $x$ solución de del PVI (\ref{syst}) con campo vectorial $f$ seis veces continuamente diferenciable en el compacto $\mathfrak{K} \subset \mathfrak{D}$. Entonce, la Fórmulas de Runge-Kutta localmente linealizadas libres de Jacobiano (\ref{Jacobian-free LLDPK scheme}) tienen error de truncamiento local
...
Similar a como se hizo en el Capítulo \ref{chapter:lldp}, la matriz particionada $E_{h_n/90}=e^{\frac{h_n}{90}\overline{H}}$ es aproximada por la aproximación ($\pf,\qf$)-Padé $P_{h_n/90}$, donde $\frac{1}{ 90}$ es el máximo común divisor de los coeficientes de Runge-Kutta $\frac{1}{5},\frac{3}{10},\frac{4}{5},\frac{8}{9} ,1$ de la Tabla \ref{ButcherTabla}. Utilizando la propiedad de flujo del operador exponencial se obtiene

\begin{align}
    P_{\frac{2}{90}h_n}& =P_{\frac{1}{90}h_n}P_{\frac{1}{90}h_n} & P_{\frac{4}{90}h_n}& =P_{\frac{2}{90}h_n}P_{\frac{2}{90}h_n}  \notag \\
    P_{\frac{8}{90}h_n}& =P_{\frac{4}{90}h_n}P_{\frac{4}{90}h_n} & P_{\frac{16}{90}h_n}& =P_{\frac{8}{90}h_n}P_{\frac{8}{90}h_n}  \notag \\
    P_{\frac{32}{90}h_n}& =P_{\frac{16}{90}h_n}P_{\frac{16}{90}h_n} & P_{\frac{80}{90}h_n}& =P_{\frac{32}{90}h_n}P_{\frac{16}{90}h_n}P_{\frac{32}{90}h_n}
    \label{flow2} \\
    P_{\frac{1}{10}h_n}& =P_{\frac{8}{90}h_n}P_{\frac{1}{90}h_n} & P_{\frac{1}{5}h_n}& =P_{\frac{1}{10}h_n}P_{\frac{1}{10}h_n}  \notag \\
    P_{\frac{2}{5}h_n}& =P_{\frac{1}{5}h_n}P_{\frac{1}{5}h_n} & P_{\frac{4}{5}h_n}& =P_{\frac{2}{5}h_n}P_{\frac{2}{5}h_n}  \notag \\
    P_{\frac{3}{10}h_n}& =P_{\frac{1}{10}h_n}P_{\frac{1}{5}h_n} & E_{1}& =P_{\frac{4}{5}h_n}P_{\frac{1}{5}h_n}\;,  \notag
\end{align}
con lo cual las cinco matrices requeridas $E_{c_j}$ son aproximadas por $P_{c_j h_n}$.

\subsection{Implementación Numérica}

En esta sección, una implementación adaptativa de las fórmulas embebidas de Runge-Kutta localmente linealizadas libres de jacobiano
(\ref{Jacobian-free LLDPK scheme}). La implementación general del esquema libre de jacobiano de orden variable con tamaño de paso, dimensión de Krylov y orden de Padé variables se esboza en Algorithm \ref{alg:integrator-fj}. Este Algoritmo es una versión adaptada del Algoritmo \ref{alg:integrator}. A continuación se detallan las particularidades de esta implementación libre de jacobiano.

{\SetAlgoNoLine
	\begin{algorithm}[htb]
		\caption{Variable-order Jacobian-free scheme with variable step size, Krylov dimension and Padé order}
		\label{alg:integrator-fj}
		\KwIn{time
			interval $[t_0, T]$, initial value $y_0$, absolute and relative tolerances $Atol$ and $Rtol$, maximum and minimum Krylov subspaces dimension $\mf_{max}$ and $\mf_{min}$, maximum step sizes $h_{max}$}
		\KwOut{$\{(t_0,y_0),\ldots,(t_n,y_n)\}$}
		$n=0$, $fail=false$, $\mf_0=\mf_{min}$, $h_{min}=16 \cdot \epsilon(t_0)$ \\
		Compute the initial step size $h_0$ by formula (\ref{hcero-fj}) \\
		\While{$t_n<T$}{
			Compute $\delta_1$ and $\delta_2$ with (\ref{deltavalue}), and set $g_1$ and $g_2$ according to (\ref{g1}) and (\ref{g2}) \label{set delta}\\
			\If{$fail=false$}{
				Run Algorithm~\ref{alg:iArnoldi} to get matrices $\widehat{H}^*_{m_n}$ and $\widehat{V}_{m_n}$ of $\widehat{\mathcal{K}}_\mf(h_n^\beta f_x(y),f(y);\delta_1)$, and set $\widehat{H}_{m_n}=\widehat{H}^*_{m_n}/h_n^\beta$ \\
			}
			Computation of $\{\widetilde{u}_1,\dots,\widetilde{u}_s\}$ by Algorithm~\ref{alg:errcontrol}\\
			Evaluation of the embedded formulas (\ref{Jacobian-free LLDPK scheme}) \\
			Estimation of the embedded formulas error $\varepsilon_y$ by formula (\ref{epsilon_y-fj}) \\
			Estimation of the new Krylov dimension $\mf_{new}$ by formula (\ref{calcmnew-fj}) \\
			Estimation of the new step size $h_{new}$ by formula (\ref{hnewcalc-fj}) \\
		...
		\nonl $\epsilon(t_n)$ denotes the spacing of the floating point number $t_n$
\end{algorithm}}
...
El cálculo adaptativo de los términos $\widetilde{u}_j$ en (\ref{Jacobian-free LLDPK scheme}) mediante la aproximación de Krylov-Padé libre jacobiano (\ref{eq:gen_kp_aprox_fj}) se esboza en Algoritmo \ref{alg:errcontrol-fj}. A continuación se describen los procedimientos para seleccionar la dimensión Krylov y el orden Padé. Con este fin, se supone que, en un paso de integración $t_n$ con un tamaño de paso $h_n$, el algoritmo de Arnoldi libre jacobiano \ref{alg:iArnoldi} se ha ejecutado con la dimensión de Krylov $\mf_n$. La aproximación de Padé es estimada como \ref{sec:pade-order}.

{\SetAlgoNoLine
	\begin{algorithm}[htb]
		\caption{Computation of $\{u_1,\dots,u_s\}$ via adaptive Jacobian-free Krylov-Padé approximations}
		...
		\While{work}{\If{$ h_n \lvert\lvert \overline{H} \rvert\rvert_\infty > 600$}{$h_n=\maxx{h_{min},600/ \lvert\lvert \overline{H} \rvert\rvert_\infty}$}
			Select the Padé order $\pf$ according to Table  \ref{table:padep} \\
			Estimate the relative error $\varepsilon_{K}$ by formula~(\ref{errrel-fj}) \\
			\eIf{$\varepsilon_{K}/\gamma_{K}< 1$}{
				...
				\eIf{$breakdown$ = $true$ \textbf{or} $\mf_n=\mf_{max}$}{
					...
					Estimate a new Krylov dimension $\mf_{new}$ by formula
					(\ref{calcmnew-fj}) \\
					Run Algorithm~\ref{alg:iArnoldiexpand} to get  matrices $\widehat{H}^*_{m_{new}}$ and $\widehat{V}_{m_{new}}$ of $\widehat{\mathcal{K}}_\mf(h_n^\beta f_x(y_n),f(y_n);\delta_1)$ and set $\widehat{H}_{m_{new}}=\widehat{H}^*_{m_{new}}/h_n^{\beta}$\\
					Set $\mf_n=\mf_{new}$ and update all the variables depending of $\mf_n$,
				}
			}
		}
		Compute $\widetilde{u}_j$ by the formula (\ref{eq:adapt_gen_kp_aprox}), for $j=1,\dots,s$\\
\end{algorithm}}

\begin{algorithm} [htb]
	\caption{Arnoldi algorithm to expand an orthonormal basis $\{v_1,\ldots,v_{\mf} \}$ of the $\mf$-th Krylov subspace $\widehat{\mathcal{K}}_\mf(\tau f_x(y),b;\delta_1)$ to an orthonormal basis $\{v_1,\ldots,v_{\mf},\ldots,v_{\mf_{new}} \}$ of the $\mf_{new}$-th Krylov subspace $\widehat{\mathcal{K}}_{\mf_{new}}(\tau f_x(y),b;\delta_1)$}
	\label{alg:iArnoldiexpand}
	\KwIn{function $g_1: \mathbb{R}^{d}\times \mathbb{R}^{d}\times \mathbb{R}_+ \to \mathbb{R}^{d}$ defined as in (\ref{eq:g_bound}), $y,b \in \mathbb{R}^{d}$, $\tau,\delta_1>0$, $\widehat{V}_{\mf}\in \mathbb{R}^{d\times \mf}$, $\widehat{H}^*_{\mf}\in\mathbb{R}^{\mf\times \mf}$, $\widehat{v}_{\mf+1}\in \mathbb{R}^{d}$, $\widehat{\hf}_{\mf+1,\mf} \ge 0$, and the new Krylov subspace dimension $\mf_{new}$}
	...
	Lines 3-16 in the Algorithm \ref{alg:iArnoldi}, but replacing the line 2 by \textbf{for} $j=\mf+1,\ldots,\mf_{new}$ \textbf{do}
\end{algorithm}
...
\begin{equation}\label{calcmnew-fj}
	...
	\end{equation}
	where $\mf_{min}$ and $\mf_{max}$ are the prescribed minimum and maximum permissible values for the Krylov dimension,
\begin{equation}
	\mf = \left\{ 
\begin{array}{cc}
\left\lfloor \mf_{n} + \maxx{\fac_{1,max} , \minn{\fac_{1,min},
		\fac_1\cdot\Delta\mf} } \right\rfloor & if \;  \varepsilon_{K}/\gamma_{K}< 1\\ 
\left\lceil \mf_{n} + \minn{\fac_{2,max} , \maxx{\fac_{2,min},
		\fac_2\cdot\Delta\mf} } \right\rceil  & otherwise%
\end{array}%
\right. \label{m_unica}
\end{equation} 
...
\begin{eqnarray}
	...
	\widehat{V}_{\mathfrak{m}}\;[P_{c_{j}h_n}]_{12}+
	\widehat{\mathfrak{h}}_{\mathfrak{m}+1,\mathfrak{m}}e_{\mathfrak{m}%
	}^{T}\;[P_{c_{j}h_n}]_{14}\rho _{4} & \text{otherwise}%
	\end{array}%
	\right. ,  \notag
\end{eqnarray}
a $u_j$, $\gamma_{K}=0.001$ un factor de seguridad,
$\Delta\mf=\log(\varepsilon_{K}/\gamma_{K})$,
$\fac_{1,max}= -\frac{\mf_n}{4}$, $\fac_{1,min}= \frac{\mf_n}{3}$, $\fac_1=\frac{1} {\log(2)}$, $\fac_{2,max}= \maxx{ 1,\frac{\mf_{n}}{3} }$, $\fac_{2,min}= 1 $ y $\fac_2=\frac{1}{\log(2)}$, $\rho_3 =\hat{v}_{m+1}$, y $\rho_4 = g_1(y_n,\hat{v}_ {m+1};\delta_1)$. En (\ref{m_unica}), el símbolo $\left\lfloor \cdot \right\rfloor$ denota la función de suelo (que devuelve el mayor entero menor o igual que un número real dado), y $\left\lceil \cdot \right\rceil$ denota la función de techo (que devuelve el menor entero mayor o igual que un número real dado). En (\ref{errrel-fj}), $ATol$ y $RTol$ son las tolerancias absolutas y relativas para integrar el PVI, $P_{h_n}$ es la matriz calculada en (\ref{flow2}), y $\widehat {\hf}_{\mf+1,\mf}$ y $\widehat{v}_{\mf+1}$ son salidas del algoritmo de Arnoldi libre de jacobiano \ref{alg:iArnoldi}. Para preservar el ordern de convergencia establecido en el Teorema \ref{Teorema Convergencia} para las fórmulas integradas (\ref{Jacobian-free LLDPK scheme}), se establece $\mf_{min}=4$.

Anteriormente, para la dimensión del subespacio de Krylov $\mathfrak{m}_{new}$ que se utlizará en el próximo paso de integración, se utilizó la expresión (\ref{calcmnew}), pero con $\varepsilon_{K} = \epsilon _{4}$ en lugar de (\ref{errrel-fj}) y con la aproximación
$\widehat{K}_{\mathfrak{m},k}^{\mathfrak{p},\mathfrak{q}}\left(
c_{j}h_{n},f_{x}(y_{n}),f(y_{n});\eta _{1},\delta _{1},\beta \right)
$ definido en (\ref{eq:gen_kp_aprox}) en lugar de (\ref{eq:adapt_gen_kp_aprox}). Las nuevas expresiones (\ref{errrel}) y (\ref{eq:adapt_gen_kp_aprox}) surgen para tener en cuenta el caso de convergencia oscilatoria de la serie de expansión \cite{sidje1998expokit}
\begin{equation*}
...
\begin{equation}
	g_1(y_n,\tau \widehat{v}_j;\delta_1)=\frac{f(y_n+\delta_1 \tau \widehat{v}_j)-f(y_n)}{\delta_1}  \;\;\; \text{and} \;\;\; g_2(y_n,\widetilde{u}_j;\delta_2)=\frac{f(y_n+\delta_2  \widetilde{u}_j)-f(y_n)}{\delta_2} \label{FDO1}
 \end{equation}
tomando
\begin{equation}\label{deltaconstraint}
	\delta_1 = h_n^{\alpha_1}, \;\;\;  \delta_2 = h_n^{\alpha_2} \;\;\; \text{and} \;\;\; \tau=h_n^{\beta},
\end{equation}
con $\alpha_1=\alpha_2=3$ y $\beta=1$. De esta forma, las fórmulas embebidas libres de jacobiano (\ref{Jacobian-free LLDPK scheme}) conservan el orden $5$ y $4$ de las fórmulas originales de Dormand y Price con un coste computacional mínimo.
...
Por lo tanto, para obtener una precisión adecuada para las aproximaciones $g_1$ y $g_2$ en (\ref{FDO1}), es necesario establecer
\begin{equation} \label{deltavalue}
	\delta_1 \tau = \sqrt{(1+||y_n||_2)\epsilon_{mach}}
	\;\;\; \text{and} \;\;\; \delta_2 = \frac{\sqrt{(1+||y_n||_2)\epsilon_{mach}}}{\epsilon_{mach}+||u_j||_2},
\end{equation}
...
\begin{equation*}
	g_1(y_n,\tau \widehat{v}_j;\delta_1)=\frac{f(y_n+\delta_1 \widehat{v}_j)-f(y_n-\delta_1 \widehat{v}_j)}{2\delta_1}  \;\;\; \text{and} \;\;\; g_2(y_n,\widetilde{u}_j;\delta_2)=\frac{f(y_n+\delta_2  \widetilde{u}_j)-f(y_n-\delta_2 \widetilde{u}_j)}{2\delta_2}
\end{equation*}
...
En resumen, en el paso \ref{set delta}, el algoritmo \ref{alg:integrator} calcula $\delta_1$ y $\delta_2$ de acuerdo con (\ref{deltavalue}) y establece de forma adaptativa la diferencia finita central o hacia adelante para $g_1$ y $g_2$ dependiendo del valor de $\alpha_1$ y $\alpha_2$. Eso es,
\begin{equation} \label{g1}
	g_{1}(y_{n},\tau \widehat{v}_{j};\delta _{1}) : \left\{ 
	\begin{array}{cc}
	\frac{f(y_{n}+\delta _{1}\widehat{v}_{j})-f(y_{n}-\delta _{1}\widehat{v}_{j})%
	}{2\delta _{1}} & \text{if }\alpha _{1}<4 \\ 
	\frac{f(y_{n}+\delta _{1}\widehat{v}_{j})-f(y_{n})}{\delta _{1}} & otherwise%
	\end{array}%
	\text{ }\right.
\end{equation}
y
\begin{equation}  \label{g2}
	...
	\frac{f(y_{n}+\delta _{2}\widetilde{u}_{j})-f(y_{n}-\delta _{2}\widetilde{u}%
		_{j})}{2\delta _{2}} & \text{if }\alpha _{2}<3 \;\;\text{and}\;\; \alpha_2+1 < \alpha_1\eta_1 \\
...
\begin{equation} \label{alpha_formulas}
    \alpha_1=ln(\delta_1)/ln(h_n) \;\;\;\;\;\; \text{and} \;\;\;\;\;\; \alpha_2=ln(\delta_2)/ln(h_n).
\end{equation}
Claramente, de acuerdo con el Teorema \ref{Teorema Convergencia}, este procedimiento produce la implementación de orden variable de las fórmulas linealizadas localmente libres de Jacobiano de Dormand y Prince descritas en el Algoritmo \ref{alg:integrator}.

\subsection{Estimación del tamaño de paso}\label{Sec:AdaptiveLLscheme}
...
\begin{equation*}
    \Delta = \begin{cases}
        ...
        \end{cases}
\end{equation*}
con $\rho = 1\mathord{.}25 \left( \frac{\varepsilon_y}{RTol} \right)^{\textcolor{red}{1/r_n}}$, siendo
\begin{equation*}
...
con $\mf_{min}=4$, $\pf_{min}=3$, $\beta=0$, $\eta_2=2$. Para el código \emph{LLDP1}, si $\alpha^{min}_1 < 4$, $\eta^{min}_1=2$, de lo contrario $\eta^{min}_1=1$. Del mismo modo, si $\alpha^{max}_1 < 4$, $\eta^{max}_1=2$, de lo contrario $\eta^{max}_1=1$. Para el código \emph{LLDP2}, $\eta^{min}_1=\eta^{max}_1=1$ siempre. Nuevamente, el símbolo $\left\lfloor \cdot \right\rfloor$ en las expresiones anteriores denota la función floor.

Observe que, solo para una de las seis ecuaciones de prueba, el integrador \emph{LLDP1} conserva el orden de convergencia de las fórmulas originales de Dormand y Prince durante todo el proceso de integración y, solo para la misma ecuación, las aproximaciones en diferencias finitas son intercambiados dentro del algoritmo de Arnoldi sin jacobiano \ref{alg:iArnoldi}. Para otras cuatro ecuaciones de prueba, el integrador \emph{LLDP1} alcanza el orden de $5$ en algunos pasos de integración, mientras que para el resto de la ecuación nunca archiva ese orden. Por otro lado, dado que la tasa de convergencia de la aproximación $g_1$ dentro del Algoritmo \ref{alg:iArnoldi} para el integrador \emph{LLDP1} es siempre mayor que la de la aproximación utilizada por el integrador \emph{LLDP2} , la tasa de convergencia del primero siempre es mayor que la del segundo integrador. Esto justifica los menores errores del código \emph{LLDP1} reportados en las Tablas \ref{tab:cuspburg} - \ref{tab:brussdnd}. Sin embargo, analizando los resultados generales para las ecuaciones de prueba consideradas, podemos afirmar que el código \emph{LLDP2} es tan efectivo como el código \emph{LLDP1}.
...
\begin{figure}
	\begin{center}
		...
		\vspace{-0.95in}
		\caption{ Comparative precision diagrams on the log-log scale (CPU time vs. relative error) for each one of codes in the integrations of the six test equations. $\vartriangle$: code \emph{JF-ode15sk1}, $\circ$ : code \emph{JF-ode15sk2}, and $\square$: code \emph{JF-LLDP45}}
		\label{lldpfj:Fig1}
	\end{center}
\end{figure}

\begin{table}
	\caption{Performance of the codes \emph{ODE15sk}, \emph{LLDP1}, \emph{LLDP2} and \emph{Exp4} in the integration of the CUSP and Burgers equations.}
	\label{tab:cuspburg}
...
\begin{table}
	\caption{Performance of the codes \emph{ODE15sk}, \emph{LLDP1}, \emph{LLDP2} and \emph{Exp4} in the integration of the Brusselator2D and GrayScott2D equations.}
...
\begin{table}
	\caption{Performance of the codes \emph{ODE15sk}, \emph{LLDP1}, \emph{LLDP2} and \emph{Exp4} in the integration of the Brusselator and DND equations.}
	\label{tab:brussdnd}
	\begin{adjustbox}{width=0.95\columnwidth,center}
		\begin{tabular}{ccccccccccc}
			\hline
...
\begin{table}
	\caption{Minimum and maximum values of $\alpha _{1}$ and $\alpha _{2}$ corresponding to the values of these parameters in the scheme \emph{LLDP1} when integrating the six test equations. $r^{min}-r^{max}$ is the range of fluctuations in the convergence rate $r$ of the integrator \emph{LLDP1} during the integration process. Symbols $+$ and $*$ indicate, respectively, interchanges of finite difference formulas inside Algorithm \ref{alg:iArnoldi} or in expression (\ref{Jacobian-free LLDPK scheme}) during the integration process.}
	\label{tab:R_LLDP1}
	\begin{adjustbox}{width=0.95\columnwidth,center}
...
\begin{table}
	\caption{Minimum and maximum values of $\alpha _{1}$ and $\alpha _{2}$ corresponding to the values of these parameters in the scheme \emph{LLDP2} when integrating the six test equations. $r^{min}-r^{max}$ is the range of fluctuations in the convergence rate $r$ of the integrator \emph{LLDP2} during the integration process. Symbol $*$ indicates interchanges of finite difference formulas in expression (\ref{Jacobian-free LLDPK scheme}) during the integration process.}
	\label{tab:R_LLDP2}
	\begin{adjustbox}{width=0.95\columnwidth,center}
		\begin{tabular}{lccccccccccc}
			& \multicolumn{3}{c}{$\alpha^{min}_{1}-\alpha^{max}_{1}$} &  & \multicolumn{3}{c}{$\alpha^{min}_{2}-\alpha^{max}_{2}$} &  & \multicolumn{3}{c}{$r^{min}-r^{max}$} \\
