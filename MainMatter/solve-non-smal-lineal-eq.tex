...
En general para PVI de grandes dimensiones la evaluación de la matriz Jacobiana $f_x$ del campo vectorial $f$ no siempre es posible, ya sea por no ser eficiente en términos de memoria u operaciones flotantes o por tener una expresión matemática difícil obtener explícitamente. En estos casos, las aproximaciones libres de Jacobiano para el producto de matriz Jacobiana por vector son necesarias~\cite{al2009complex,knoll2004jacobian}. Por construcción, la discretización Lineal Local~(\ref{ODE-LLA-4}) contiene productos de la matriz Jacobiana por vector $f_x(y)b$ que pueden ser aproximados por diferencias finitas de orden 1 \redmark{o} 2. A modo ilustrativo se tomará la diferencia finita hacia adelante
\begin{equation*}
